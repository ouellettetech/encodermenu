...
import demo.freesans20 as freesans20  # Font to use
from demo.picolcd import LCD_1inch14

from demo.writer import Writer
...
patterns = wrap_menu(
    [("Chaser", yellow), ("Blue", blue), ("Rainbow", red), ("Back!", back)]
)
main_menu = wrap_menu(
    [("Patterns", patterns), ("trees", trees), ("Brightness", brightness)]
)
pixels = wrap_menu([("Back!", back)])
settime = wrap_menu(
    [
        ("Time wizard", timewizard),
        ("Date wizard", datewizard),
        ("Write to clock", setclockinfo),
        ("Show datetime", datetimeinfo),
        ("Back..", back),
    ]
)
...
root_menu = wrap_menu(
    [
        ("Set DateTime", settime),
        ("Colour 1", colour1),
        ("Speed", speed),
        ("Patterns", patterns),
        ("Open door", opendoor),
        ("Close door", closedoor),
        ("Automate door", automate),
    ]
)
